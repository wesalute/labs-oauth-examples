...
export function authorizeURL(host, client_id) {
  ...
  const props = {
    client_id: clients[client_id].id,
  ...
  console.log("Constructing authorization URL with:", props);
...
export async function getAccessToken(host, code, client_id) {
  ...
  console.log("Exchanging access code for access token:", props);

  try {
    const token = await client.getToken(props);
    console.log("Received token:", token.token);
    return token.token;
  }
  catch (e) {
    console.log(e);
...
// Step 3: Refresh Access Token with Refresh Token
export async function refreshAccessToken(client_id, refresh_token) {
  ...
  try {
    ...
    const token = await tokenObject.refresh();
    console.log("Received new token:", token.token);
    return token.token;
  } catch (e) {
    // Handle errors (e.g., token expired, network errors)
    console.error('Error refreshing access token:', e.data.payload);
    return;
  }
}
